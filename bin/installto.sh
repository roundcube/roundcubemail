...
if (strtolower($input) == 'y') {
    ...
    if (is_dir(INSTALL_PATH . 'vendor') && !is_file("$target_dir/composer.json") || vendor_dir_untouched($target_dir)) {
        $dirs[] = 'vendor';
    }
...
function vendor_dir_untouched($target_dir)
{
    system('grep -q "generated by Roundcube install" ' . $target_dir . '/vendor/autoload.php 2>/dev/null', $exit_code);
    return $exit_code === 0;
}
