...
if ($RCI->configured) {
    ...
    if (is_file(INSTALL_PATH . 'composer.json') && is_readable(INSTALL_PATH . 'composer.json-dist')) {
        ...
        if (!vendor_dir_untouched()) {
            $exit_code = 1;
            if ($composer_bin = find_composer()) {
                echo "Executing " . $composer_bin . " to update dependencies...\n";
                echo system('(cd ' . INSTALL_PATH . "; $composer_bin update --no-dev)", $exit_code);
            }
            if ($exit_code != 0) {
...
function vendor_dir_untouched()
{
    system('grep -q "generated by Roundcube install" ' . INSTALL_PATH . 'vendor/autoload.php 2>/dev/null', $exit_code);
    return $exit_code === 0;
}
