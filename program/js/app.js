...
function rcube_webmail() {
    ...
    this.set_env = function (p, value) {
        if (p != null && typeof p === 'object' && !value) {
            for (var n in p) { this.env[n] = p[n]; }
        } else { this.env[p] = value; }
    };

    // add a localized label to the client environment
    this.add_label = function (p, value) {
        if (typeof p == 'string') { this.labels[p] = value; } else if (typeof p == 'object') { $.extend(this.labels, p); }
    };
    ...
    this.register_button = function (command, id, type, act, sel, over) {
        ...
        if (!this.buttons[command]) { this.buttons[command] = []; }
    ...
    this.register_menu_button = function (button, menu_id) {
        if (this.menu_buttons[menu_id]) {
            ...
            $('#' + menu_id).find('a').each(function () {
                ...
                if (onclick && String(onclick).match(/rcmail\.command\(\'([^']+)/)) { command = RegExp.$1; } else { command = function () { return link.is('.active'); }; }

                commands.push(command);
            });

            if (commands.length) { this.menu_buttons[menu_id] = [[button], commands]; }
        }
    ...
    this.add_element = function (elm, container) {
        if (this.gui_containers[container] && this.gui_containers[container].jquery) { this.gui_containers[container].append(elm); }
    };
    ...
    this.register_command = function (command, callback, enable) {
        ...
        if (enable) { this.enable_command(command, true); }
    };
    ...
    this.init = function () {
        ...
        if (!this.env.blankpage) { this.env.blankpage = 'javascript:false;'; }

        // find all registered gui containers
        for (n in this.gui_containers) { this.gui_containers[n] = $('#' + this.gui_containers[n]); }

        // find all registered gui objects
        for (n in this.gui_objects) { this.gui_objects[n] = rcube_find_object(this.gui_objects[n]); }
        ...
        if (this.env.permaurl) { this.enable_command('permaurl', 'extwin', true); }
        ...
        switch (this.task) {
            case 'mail':
                ...
                if (this.gui_objects.messagelist) {
                    ...
                    if (searchfilter && searchfilter != 'ALL') { this.filter_mailbox(searchfilter); } else { this.command('list'); }
                ...
                if (this.env.action == 'show' || this.env.action == 'preview') {
                    ...
                    if (this.env.drafts_mailbox) { this.env.compose_commands.push('savedraft'); }
                    ...
                    if (this.env.is_message) {
                        ...
                        if (this.env.list_post) { this.enable_command('reply-list', true); }
                    }
                ...
                if (!this.is_framed() && !this.env.extwin) { this.browser_capabilities_check(); }
            ...
            case 'addressbook':
                ...
                if (this.gui_objects.folderlist) { this.env.contactfolders = $.extend($.extend({}, this.env.address_sources), this.env.contactgroups); }
                ...
                if (this.gui_objects.editform) {
                    ...
                    if (this.env.action == 'add' || this.env.action == 'edit' || this.env.action == 'search') { this.init_contact_form(); }
            ...
            case 'login':
                ...
                if (input_user.val() == '') { input_user.focus(); } else { $('#rcmloginpwd').focus(); }
        ...
        if (bw.ie) { $('input[type=file]').keydown(function (e) { if (e.keyCode == '13') { e.preventDefault(); } }); }
        ...
        // show message
        if (this.pending_message) { this.display_message.apply(this, this.pending_message); }
      ...
      && this.gui_objects.folderlist != this.gui_objects.addressbookslist
        ...
        for (n in this.onloads) {
            if (typeof this.onloads[n] === 'string') { eval(this.onloads[n]); } else if (typeof this.onloads[n] === 'function') { this.onloads[n](); }
        }
    ...
    this.log = function (msg) {
        if (this.env.devel_mode && window.console && console.log) { console.log(msg); }
    };
    ...
    this.command = function (command, props, obj, event, allow_disabled) {
        ...
        if (obj && obj.blur && !(event && rcube_event.is_keyboard(event))) { obj.blur(); }

        // do nothing if interface is locked by another command
        // with exception for searching reset and menu
        if (this.busy && !(command == 'reset-search' && this.last_command == 'search') && !command.match(/^menu-/)) { return false; }
        ...
        if (!allow_disabled && !this.commands[command]) {
            ...
            if (this.is_framed()) { parent.rcmail.command(command, props); }
        ...
        if (!this.command_aborted && this.triggerEvent('after' + command, props) === false) { ret = false; }

        this.triggerEvent('actionafter', { props: props, action: command, aborted: this.command_aborted, ret: ret, originalEvent: event });

        if (ret === false) { return false; }

        if ((obj && ret !== true) || this.command_aborted === true) { return false; }
    ...
    this.command_handler = function (command, props, obj, event) {
        ...
        switch (command) {
            ...
            case 'permaurl':
                if (obj && obj.href && obj.target) { return true; }
                if (this.env.permaurl) { parent.location.href = this.env.permaurl; }
                break;
            ...
            case 'change-format':
                ...
                if (this.env.action == 'preview') { url = url.replace(/_action=show/, '_action=preview') + '&_framed=1'; }
            ...
            case 'close':
                if (this.env.extwin) { window.close(); }
                break;
            ...
            case 'list':
                ...
                if (this.env.action == 'compose' && this.env.extwin) {
                    ...
                    this.set_button_titles();
                } else if (this.task == 'addressbook') { this.list_contacts(props); }
                break;
            ...
            case 'expunge':
                if (this.env.exists) { this.expunge_mailbox(this.env.mailbox); }
                break;
            ...
            case 'show':
                if (this.task == 'mail') {
                    ...
                    if (uid && (!this.env.uid || uid != this.env.uid)) {
                        ...
                        if (mbox == this.env.drafts_mailbox) { this.open_compose_step({ _draft_uid: uid, _mbox: mbox }); } else { this.show_message(uid); }
                    }
                } else if (this.task == 'addressbook') {
                    cid = props ? props : this.get_single_cid();
                    if (cid && !(this.env.action == 'show' && cid == this.env.cid)) { this.load_contact(cid, 'show'); }
            ...
            case 'add':
                if (this.task == 'addressbook') { this.load_contact(0, 'add'); } else if (this.task == 'settings' && this.env.action == 'responses') { this.load_response(0, 'add-response'); } else if (this.task == 'settings') { this.load_identity(0, 'add-identity'); }
                break;

            case 'edit':
                if (this.task == 'addressbook' && (cid = this.get_single_cid())) { this.load_contact(cid, 'edit'); } else if (this.task == 'mail' && (uid = this.get_single_uid())) {
            ...
            case 'save':
                ...
                if (form) {
                    ...
                    if (parent.rcmail && parent.rcmail.env.source) { form.action = this.add_url(form.action, '_orig_source', parent.rcmail.env.source); }
            ...
            case 'delete':
                // mail task
                if (this.task == 'mail') { this.delete_messages(event); }
                // addressbook task
            ...
            case 'copy':
                if (this.task == 'mail') { this.copy_messages(props, event); } else if (this.task == 'addressbook') { this.copy_contacts(props, event); }
                break;

            case 'mark':
                if (props) { this.mark_message(props); }
                break;
            ...
            case 'toggle_flag':
                ...
                if (uid = props) {
                    ...
                    if (flag == 'flagged') {
                        if (this.message_list.rows[uid].flagged) { flag = 'unflagged'; }
                    }
                    // toggle read/unread
                    else if (this.message_list.rows[uid].deleted) { flag = 'undelete'; } else if (!this.message_list.rows[uid].unread) { flag = 'unread'; }
            ...
            case 'download-attachment':
                ...
                if (command != 'download-attachment' && mimetype && this.env.mimetypes && $.inArray(mimetype, this.env.mimetypes) >= 0) {
                    ...
                    if (this.open_window(this.url('get', $.extend({ _frame: 1, _framed: 0 }, params)))) { return true; }
                }
            ...
            case 'select-all':
                ...
                if (props == 'invert') { list.invert_selection(); } else { list.select_all(props == 'page' ? '' : props); }
            ...
            case 'nextmessage':
                if (this.env.next_uid) { this.show_message(this.env.next_uid, false, this.env.action == 'preview'); }
                break;

            case 'lastmessage':
                if (this.env.last_uid) { this.show_message(this.env.last_uid); }
                break;

            case 'previousmessage':
                if (this.env.prev_uid) { this.show_message(this.env.prev_uid, false, this.env.action == 'preview'); }
                break;

            case 'firstmessage':
                if (this.env.first_uid) { this.show_message(this.env.first_uid); }
                break;
            ...
            case 'compose':
                ...
                // modify url if we're in addressbook
                    ...
                    if (props && props.indexOf('@') > 0) {
                        ...
                        if (props) { a_cids.push(props); }
                        // get selected contacts
                        else if (this.contact_list) { a_cids = this.contact_list.get_selection(); }
            ...
            case 'send':
                if (!props.nocheck && !this.env.is_sent && !this.check_compose_input(command)) { break; }
            ...
            case 'send-attachment':
                ...
                if (!(flag = this.upload_file(props || this.gui_objects.uploadform, 'upload'))) {
                    if (flag !== false) { this.alert_dialog(this.get_label('selectimportfile')); }
                    aborted = true;
                }
                break;
            ...
            case 'reply':
                if (uid = this.get_single_uid()) {
                    ...
                    { url._all = (!props && this.env.reply_all_mode == 1 && this.commands['reply-list'] ? 'list' : 'all'); } else if (command == 'reply-list') { url._all = 'list'; }
            ...
            case 'forward':
                ...
                if (uids.length) {
                    ...
                    if (command == 'forward-attachment' || (!props && this.env.forward_attachment) || uids.length > 1) { url._attachment = 1; }
                    this.open_compose_step(url);
                }
                break;
            ...
            case 'print':
                if (this.task == 'addressbook') {
                    if (uid = this.get_single_cid()) {
                        ...
                        if (this.env.source) { url += '&_source=' + urlencode(this.env.source); }
                    ...
                    if (this.open_window(url, true, true)) {
                        if (this.env.action != 'show' && this.env.action != 'get') { this.mark_message('read', uid); }
                    }
                }
                break;

            case 'viewsource':
                if (uid = this.get_single_uid()) { this.open_window(this.url('viewsource', this.params_from_uid(uid)), true, true); }
                break;
            ...
            case 'reset-search':
                ...
                if (s && this.env.action == 'compose') {
                    if (this.contact_list) { this.list_contacts_clear(); }
            ...
            case 'pushgroup':
                ...
                if (obj && event) { rcube_event.cancel(event); }
            ...
            case 'popgroup':
                if (this.env.address_group_stack.length) {
                    ...
                    if (old.search_request) {
                        ...
                        if (old.search && this.gui_objects.qsearchbox) { $(this.gui_objects.qsearchbox).val(old.search); }
                        this.env.search_request = old.search_request;
                        this.list_contacts_remote(null, null, this.env.current_page = old.page);
                    } else { this.list_contacts(props.source, this.env.address_group_stack[this.env.address_group_stack.length - 1].id); }
                }
                break;
            ...
            case 'import':
                var dialog = $('<iframe>').attr('src', this.url('import', { _framed: 1, _target: this.env.source })),
                    import_func = function (e) {
                        ...
                        if (win.rcmail.gui_objects.importformmap) { form = win.rcmail.gui_objects.importformmap; } else { form = win.rcmail.gui_objects.importform; }
                    ...
                    close_func = function (event, ui) {
                        $(this).remove();
                        if (ref.import_state == 'reload') { ref.command('list'); }
                    };
    ...
    this.set_busy = function (a, message, id) {
        ...
        if (this.gui_objects.editform) { this.lock_form(this.gui_objects.editform, a); }
    ...
    this.get_label = function (name, domain) {
        if (domain && this.labels[domain + '.' + name]) { return this.labels[domain + '.' + name]; } else if (this.labels[name]) { return this.labels[name]; } else { return name; }
    };
    ...
    this.switch_task = function (task) {
        ...
        if (this.task === task && task != 'mail') { return; }

        var url = this.get_task_url(task);

        if (action) { url += '&_action=' + action; }

        if (task == 'mail') { url += '&_mbox=INBOX'; } else if (task == 'logout') {
    ...
    this.get_task_url = function (task, url) {
        if (!url) { url = this.env.comm_path; }

        if (url.match(/[?&]_task=[a-zA-Z0-9_-]+/)) { return url.replace(/_task=[a-zA-Z0-9_-]+/, '_task=' + task); } else { return url.replace(/\?.*$/, '') + '?_task=' + task; }
    };

    this.reload = function (delay) {
        if (this.is_framed()) { parent.rcmail.reload(delay); } else if (delay) { setTimeout(function () { ref.reload(); }, delay); } else if (window.location) { location.href = this.url('', { _extwin: this.env.extwin }); }
    };
    ...
    this.add_url = function (url, name, value) {
        ...
        if (/(\?.*)$/.test(url)) {
            ...
            if (datax.test(urldata)) { urldata = urldata.replace(datax, RegExp.$2 + name + '=' + value); } else { urldata += '&' + name + '=' + value; }
    ...
    this.save_pref = function (prop) {
        ...
        if (prop.session) { request._session = prop.session; }
        if (prop.env) { this.env[prop.env] = prop.value; }
    ...
    this.drag_start = function (list) {
        ...
        if (this.treelist) { this.treelist.drag_start(); }
    };

    this.drag_end = function (e) {
        var list, model;

        if (this.treelist) { this.treelist.drag_end(); }

        // execute drag & drop action when mouse was released
        if (list = this.message_list) { model = this.env.mailboxes; } else if (list = this.contact_list) { model = this.env.contactfolders; }
        ...
        if (this.drag_active && model && this.env.last_folder_target && !rcube_event.is_keyboard(e)) {
            ...
            if (this.contact_list) {
                if (!this.contacts_drag_menu(e, target)) { this.command('move', target); }
            } else if (!this.drag_menu(e, target)) { this.command('move', target); }
        }
    ...
    this.drag_move = function (e) {
        if (this.gui_objects.folderlist) {
            ...
            if (this.contact_list && this.contact_list.draglayer) { oldclass = this.contact_list.draglayer.attr('class'); }
            ...
            if (layerclass != oldclass && this.contact_list && this.contact_list.draglayer) { this.contact_list.draglayer.attr('class', layerclass); }
        }
    };

    this.collapse_folder = function (name) {
        if (this.treelist) { this.treelist.toggle(name); }
    };

    this.folder_collapsed = function (node) {
        if (this.folder_collapsed_timer) { clearTimeout(this.folder_collapsed_timer); }
        ...
        if (node.collapsed) {
            ...
            if (!node.virtual && this.env.mailbox && this.env.mailbox.startsWith(node.id + this.env.delimiter)) { this.command('list', node.id); }
        }

        if (!this.drag_active) {
            if (old !== this.env[prefname]) { this.folder_collapsed_timer = setTimeout(function () { ref.command('save-pref', { name: prefname, value: ref.env[prefname] }); }, 10); }
    ...
    this.doc_mouse_up = function (e) {
        ...
        if ($(target).closest('.ui-dialog, .ui-widget-overlay').length) { return; }
        ...
        if (window.rcube_list_widget && rcube_list_widget._instances.length) {
            $.each(rcube_list_widget._instances, function (i, list) {
                if (list && !rcube_mouse_is_over(e, list.list.parentNode)) { list.blur(); }
            });
        }
        ...
        if (this.buttons_sel) {
            for (id in this.buttons_sel) {
                if (typeof id !== 'function') { this.button_out(this.buttons_sel[id], id); }
            }
            this.buttons_sel = {};
        }
    ...
    // Helper method to move focus to the next/prev active menu item
        var focus_menu_item = function (dir) {
            ...
            if (ref.focused_menu && (obj = $('#' + ref.focused_menu))) {
                ...
                if (!item.length) { item = obj.find(':focus').closest('ul')[mod]().has(':not([aria-disabled=true])').find('a,input')[limit](); }
    ...
    this.list_keypress = function (list, conf) {
        ...
        if (list.key_pressed == list.DELETE_KEY || list.key_pressed == list.BACKSPACE_KEY) { this.command(conf && conf.del ? conf.del : 'delete'); } else if (list.key_pressed == 33) { this.command(conf && conf.prev ? conf.prev : 'previouspage'); } else if (list.key_pressed == 34) { this.command(conf && conf.next ? conf.next : 'nextpage'); }
    };
    ...
    // On Enter open the message in list layout mode (no preview frame)
        if (list.key_pressed == list.ENTER_KEY && !this.env.contentframe) { this.command('show'); } else { this.list_keypress(list); }
    };
    ...
    this.msglist_select = function (list) {
        ...
        if (selected) {
            if (isDraft) {
                ...
                if (!msg.ml) { this.enable_command('reply-list', false); }
            }
        }
        ...
        if (selected || (selected_count && selected_count != list.rowcount)) { this.select_all_mode = false; }
    ...
    this.msglist_get_preview = function () {
        ...
        if (uid && this.env.contentframe && !this.drag_active) { this.show_message(uid, false, true); } else if (this.env.contentframe) { this.show_contentframe(false); }
    };

    this.msglist_expand = function (row) {
        if (this.env.messages[row.uid]) { this.env.messages[row.uid].expanded = row.expanded; }
    ...
    this.msglist_set_coltypes = function (list) {
        ...
        if ((found = $.inArray('flag', this.env.listcols)) >= 0) { this.env.flagged_col = found; }

        if ((found = $.inArray('subject', this.env.listcols)) >= 0) { this.env.subject_col = found; }
    ...
    this.msglist_setup = function (layout) {
        ...
        if (ret = this.triggerEvent('msglist_layout', layout)) { layout = ret; }

        listcols = this.env[layout == 'widescreen' ? 'listcols_widescreen' : 'listcols'];

        if (layout == 'widescreen' && !this.env.threading) { listcols = $.grep(listcols, function (value) { return value != 'threads'; }); }
    ...
    this.open_window = function (url, small, toolbar) {
        ...
        if (this.env.standard_windows) { var extwin = window.open(url, wname); } else {
    ...
    this.init_message_row = function (row) {
        ...
        if (uid && this.env.messages[uid]) { $.extend(row, this.env.messages[uid]); }
        ...
        if (this.env.status_col != null) { row.msgicon = document.getElementById('msgicn' + row.id); } else { row.msgicon = row.icon; }
    ...
    this.add_message_row = function (uid, cols, flags, attop) {
        ...
        if (flags.mbox != this.env.mailbox && !flags.skip_mbox_check) { return false; }
        ...
        if (this.message_list.rows[uid]) { return false; }

        if (!this.env.messages[uid]) { this.env.messages[uid] = {}; }
        ...
        if (layout == 'widescreen') { this.env.status_col = null; } else if ((n = $.inArray('status', listcols)) >= 0) { this.env.status_col = n; }
        ...
        // update selection
        if (message.selected && !list.in_selection(uid)) { list.selection.push(uid); }
        ...
        if (this.env.threading) {
            ...
            if (flags.unread_children && flags.seen && !message.expanded) { row_class += ' unroot'; }

            if (flags.flagged_children && !message.expanded) { row_class += ' flaggedroot'; }
        }
        ...
        for (n in listcols) {
            ...
            if (c == 'flag') {
                ...
                if (flags.attachmentClass) { html = '<span class="' + flags.attachmentClass + '" title="' + label + '"></span>'; } else if (flags.ctype == 'multipart/report') { html = '<span class="report"></span>'; } else if (flags.ctype == 'multipart/encrypted' || flags.ctype == 'application/pkcs7-mime') { html = '<span class="encrypted"></span>'; } else if (flags.hasattachment || (!flags.hasnoattachment && /application\/|multipart\/(m|signed)/.test(flags.ctype))) { html = '<span class="attachment" title="' + label + '"></span>'; } else { html = '&nbsp;'; }
            } else if (c == 'status') {
                label = '';
                if (flags.deleted) {
                    css_class = 'deleted';
        ...
        if (layout == 'widescreen') { row = this.widescreen_message_row(row, uid, message); }
    ...
    this.widescreen_message_row = function (row, uid, message) {
        ...
        $.each(this.env.widescreen_list_template, function () {
            if (!ref.env.threading && this.className == 'threads') { return; }
            ...
            for (i = 0; this.cells && i < this.cells.length; i++) {
                for (n = 0; row.cols && n < row.cols.length; n++) {
                    if (this.cells[i] == row.cols[n].className) {
                        ...
                        if (this.className == 'subject' && domcol.className != 'subject') { domcol.className += ' skip-on-drag'; }
                        if (col.innerHTML) { domcol.innerHTML = col.innerHTML; }
    ...
    this.set_list_sorting = function (sort_col, sort_order) {
        ...
        if (sort_new) { $('#rcm' + sort_new).addClass('sorted' + sort_order); }
    ...
    this.set_list_options = function (cols, sort_col, sort_order, threads, layout) {
        ...
        if (sort_col === undefined) { sort_col = this.env.sort_col; }
        ...
        if (update) { this.list_mailbox('', '', sort_col + '_' + sort_order, post_data); }
    };
    ...
    this.show_message = function (id, safe, preview) {
        ...
        if (preview) { this.preview_id = id; }

        if (preview && String(target.location.href).indexOf(url) >= 0) {
            this.show_contentframe(true);
        } else {
            if (!preview && this.env.message_extwin && !this.env.extwin) { this.open_window(url, true); } else {
                ...
                if (safe && document.referrer && window.history.replaceState) { window.history.replaceState({}, '', document.referrer); }
    ...
    this.set_unread_message = function (id, folder) {
        ...
        if (!self.message_list) { self = self.opener(); }

        if (!self && window.parent) { self = parent.rcmail; }

        if (!self || !self.message_list) { return; }

        // this may fail in multifolder mode
        if (self.set_message(id, 'unread', false) === false) { self.set_message(id + '-' + folder, 'unread', false); }
        ...
        if (frame = this.get_frame_element(name)) {
            if (!show && (win = this.get_frame_window(name))) {
                if (win.location.href.indexOf(this.env.blankpage) < 0) {
                    if (win.stop) { win.stop(); } else // IE
                    { win.document.execCommand('Stop'); }

                    win.location.href = this.env.blankpage;
                }
            } else if (!bw.safari && !bw.konq) { $(frame)[show ? 'show' : 'hide'](); }
        }
    ...
    this.get_frame_element = function (id) {
        ...
        if (id && (frame = document.getElementById(id))) { return frame; }
    };

    this.get_frame_window = function (id) {
        var frame = this.get_frame_element(id);

        if (frame && frame.name && window.frames) { return window.frames[frame.name]; }
    };

    this.lock_frame = function (target) {
        var rc = this.is_framed() ? parent.rcmail : this;

        if (!rc.env.frame_lock) { rc.env.frame_lock = rc.set_busy(true, 'loading'); }
    ...
    this.list_page = function (page) {
        ...
        if (page > 0 && page <= this.env.pagecount) {
            ...
            if (this.task == 'addressbook' || this.contact_list) { this.list_contacts(this.env.source, this.env.group, page); } else if (this.task == 'mail') { this.list_mailbox(this.env.mailbox, page); }
        }
    };
    ...
    this.filter_mailbox = function (filter) {
        if (this.filter_disabled) { return; }
    ...
    this.refresh_list = function () {
        ...
        if (this.message_list) { this.message_list.clear_selection(); }
    };
    ...
    this.list_mailbox = function (mbox, page, sort, url, update_only) {
        ...
        if (!mbox) { mbox = this.env.mailbox ? this.env.mailbox : 'INBOX'; }
        ...
        // also send search request to get the right messages
        else if (this.env.search_request) { url._search = this.env.search_request; }

        if (!update_only) {
            // unselect selected messages and clear the list and message data
            this.clear_message_list();

            if (mbox != this.env.mailbox || (mbox == this.env.mailbox && !page && !sort)) { url._refresh = 1; }
        ...
        if (this.env.uid) { url._uid = this.env.uid; }
    ...
    this.clear_message_list = function () {
        ...
        this.show_contentframe(false);
        if (this.message_list) { this.message_list.clear(true); }
    };
    ...
    this.update_selection = function () {
        ...
        for (i in selected) {
            if (rows[selected[i]]) { selection.push(selected[i]); }
        }
        ...
        try {
            ...
            if (id && !list.in_selection(id)) { this.show_contentframe(false); }
        } catch (e) {}
    };
    ...
    this.expand_threads = function () {
        if (!this.env.threading || !this.env.autoexpand_threads || !this.message_list) { return; }
    ...
    // #1487752
        if (mbox && mbox != this.env.mailbox) { return false; }

        for (var n = 0, len = roots.length; n < len; n++) { this.add_tree_icons(roots[n]); }
        this.expand_threads();
    };
    ...
    this.add_tree_icons = function (root) {
        ...
        if (root) { row = rows[root] ? rows[root].obj : null; } else { row = this.message_list.tbody.firstChild; }
            ...
            if (row.nodeType == 1 && (r = rows[row.uid])) {
                if (r.depth) {
                    for (i = tmp.length - 1; i >= 0; i--) {
                        ...
                        if (len > r.depth) {
                            pos = len - r.depth;
                            if (!(tmp[i][pos] & 2)) { tmp[i][pos] = tmp[i][pos] ? tmp[i][pos] + 2 : 2; }
                        } else if (len == r.depth) {
                            if (!(tmp[i][0] & 2)) { tmp[i][0] += 2; }
                        }
                        if (r.depth > len) { break; }
                    }
                    ...
                    if (root && row != rows[root].obj) { break; }
                }
            }
            row = row.nextSibling;
        }
    ...
    this.set_tree_icons = function (uid, tree) {
        ...
        for (i = 0; i < len; i++) {
            if (tree[i] > 2) { divs.push({ 'class': 'l3', width: 15 }); } else if (tree[i] > 1) { divs.push({ 'class': 'l2', width: 15 }); } else if (tree[i] > 0) { divs.push({ 'class': 'l1', width: 15 }); }
            // separator div
            else if (divs.length && !divs[divs.length - 1]['class']) { divs[divs.length - 1].width += 15; } else { divs.push({ 'class': null, width: 15 }); }
        }

        for (i = divs.length - 1; i >= 0; i--) {
            if (divs[i]['class']) { html += '<div class="tree ' + divs[i]['class'] + '" />'; } else { html += '<div style="width:' + divs[i].width + 'px" />'; }
        }

        if (html) { $('#rcmtab' + this.html_identifier(uid, true)).html(html); }
    };

    // update parent in a thread
    this.update_thread_root = function (uid, flag) {
        if (!this.env.threading) { return; }
    ...
    this.update_thread = function (uid) {
        if (!this.env.threading || !this.message_list.rows[uid]) { return 0; }
        ...
        while (row) {
            if (row.nodeType == 1 && (r = rows[row.uid])) {
                ...
                if (!r.depth) { // a new root
                    ...
                    if (r.obj.style.display == 'none') { $(r.obj).show(); }
                } else {
                    if (r.depth == depth) { r.parent_uid = parent; }
                    if (r.unread && roots.length) { roots[roots.length - 1].unread_children++; }
                }
            }
            row = row.nextSibling;
        }
    ...
    this.delete_excessive_thread_rows = function () {
        ...
        while (row) {
            if (row.nodeType == 1 && (r = rows[row.uid])) {
                ...
                if (!cnt) { this.message_list.remove_row(row.uid); }
            }
            row = row.nextSibling;
        }
    };
    ...
    this.set_message_icon = function (uid) {
        ...
        if (row.icon) {
            ...
            if (row.deleted) {
                ...
                css_class += ' unread';
                label += this.get_label('unread') + ' ';
            } else if (row.unread_children) { css_class += ' unreadchildren'; }
    ...
    this.set_message_status = function (uid, flag, status) {
        ...
        if (flag == 'unread') {
            if (row.unread != status) { this.update_thread_root(uid, status ? 'unread' : 'read'); }
        } else if (flag == 'flagged') {
            this.update_thread_root(uid, status ? 'flagged' : 'unflagged');
        }

        if ($.inArray(flag, ['unread', 'deleted', 'replied', 'forwarded', 'flagged']) > -1) { row[flag] = status; }
    };
    ...
    this.set_message = function (uid, flag, status) {
        ...
        if (flag) { this.set_message_status(uid, flag, status); }

        if ($.inArray(flag, ['unread', 'deleted', 'flagged']) > -1) { $(row.obj)[row[flag] ? 'addClass' : 'removeClass'](flag); }
    ...
    this.copy_messages = function (mbox, event, uids) {
        ...
        if (!mbox || mbox == this.env.mailbox) { return; }
    ...
    this.move_messages = function (mbox, event, uids) {
        ...
        // exit if current or no mailbox specified
        if (!mbox || (mbox == this.env.mailbox && !this.is_multifolder_listing())) { return; }
        ...
        // show wait message
        if (this.env.action == 'show') { lock = this.set_busy(true, 'movingmessage'); }
        ...
        if (this.env.action != 'show') { this.show_contentframe(false); }
    };
    ...
    this.delete_messages = function (event, uid) {
        ...
        else if (!trash || this.env.mailbox == trash) { this.permanently_remove_messages(uid); }
        // we're in Junk folder and delete_junk is enabled
        else if (this.env.delete_junk && this.env.junk_mailbox && this.env.mailbox == this.env.junk_mailbox) { this.permanently_remove_messages(uid); }
        // if there is a trash mailbox defined and we're not currently in it
            ...
            } else { this.move_messages(trash, event, uid ? [uid] : null); }
        }
    ...
    this.with_selected_messages = function (action, post_data, lock, http_action) {
        ...
        if (this.message_list) {
            ...
            if (selection === '*') { selection = this.message_list.get_selection(); } else if (!Array.isArray(selection)) { selection = String(selection).split(','); }
            ...
            for (n = 0, len = selection.length; n < len; n++) {
                ...
                if (remove) { this.message_list.remove_row(id, display_next && n == selection.length - 1); }
            }

            // make sure there are no selected rows
            if (!display_next && remove) { this.message_list.clear_selection(); }
        ...
        if (count < 0) { post_data._count = (count * -1); }
        // remove threads from the end of the list
        else if (count > 0 && remove) { this.delete_excessive_thread_rows(); }

        if (!remove) { post_data._refresh = 1; }
    ...
    this.selection_post_data = function (data) {
        ...
        if (!data._uid) { data._uid = this.env.uid ? [this.env.uid] : this.message_list.get_selection(); }

        data._mbox = this.env.mailbox;
        data._uid = this.uids_to_list(data._uid);

        if (this.env.action) { data._from = this.env.action; }

        // also send search request to get the right messages
        if (this.env.search_request) { data._search = this.env.search_request; }

        if (this.env.display_next && this.env.next_uid) { data._next_uid = this.env.next_uid; }
    ...
    this.mark_message = function (flag, uid) {
        ...
        if (uid) { a_uids.push(uid); } else if (this.env.uid) { a_uids.push(this.env.uid); } else if (list) { a_uids = list.get_selection(); }

        if (!list) { r_uids = a_uids; } else {
            list.focus();
        ...
        // nothing to do
        if (!r_uids.length && !this.select_all_mode) { return; }
    ...
    this.toggle_read_status = function (flag, a_uids) {
        ...
        for (i = 0; i < len; i++) { this.set_message(a_uids[i], 'unread', (flag == 'unread' ? true : false)); }
    ...
    this.toggle_flagged_status = function (flag, a_uids) {
        ...
        for (i = 0; i < len; i++) { this.set_message(a_uids[i], 'flagged', (flag == 'flagged' ? true : false)); }

        if (this.env.action == 'show' || $.inArray(this.preview_id, a_uids) >= 0) { $(win.document.body)[flag == 'flagged' ? 'addClass' : 'removeClass']('status-flagged'); }
    ...
    this.toggle_delete_status = function (a_uids) {
        ...
        if (len == 1) {
            if (!this.message_list || (rows[a_uids[0]] && !rows[a_uids[0]].deleted)) { this.flag_as_deleted(a_uids); } else { this.flag_as_undeleted(a_uids); }
        ...
        if (all_deleted) { this.flag_as_undeleted(a_uids); } else { this.flag_as_deleted(a_uids); }
    ...
    this.flag_as_undeleted = function (a_uids) {
        ...
        for (i = 0; i < len; i++) { this.set_message(a_uids[i], 'deleted', false); }
    ...
    this.flag_as_deleted = function (a_uids) {
        ...
        for (var i = 0, len = a_uids.length; i < len; i++) {
            uid = a_uids[i];
            if (rows[uid]) {
                if (rows[uid].unread) { r_uids[r_uids.length] = uid; }
        ...
        if (this.env.skip_deleted && list) {
            if (!display_next || !list.rowcount) { list.clear_selection(); }
            if (count < 0) { post_data._count = (count * -1); } else if (count > 0)
            // remove threads from the end of the list
            { this.delete_excessive_thread_rows(); }
        }

        // set of messages to mark as seen
        if (r_uids.length) { post_data._ruid = this.uids_to_list(r_uids); }

        if (this.env.skip_deleted && this.env.display_next && this.env.next_uid) { post_data._next_uid = this.env.next_uid; }
    ...
    this.flag_deleted_as_read = function (uids) {
        ...
        if (typeof uids == 'string') { uids = uids.split(','); }

        for (i = 0, len = uids.length; i < len; i++) {
            uid = uids[i];
            if (rows[uid]) { this.set_message(uid, 'unread', false); }
        }
    };
    ...
    this.uids_to_list = function (uids) {
        if (this.select_all_mode) { return '*'; }

        // multi-folder list of uids cannot be passed as a string (#6845)
        if (Array.isArray(uids) && (uids.length == 1 || String(uids[0]).indexOf('-') == -1)) { uids = uids.join(','); }
    ...
    this.init_pagejumper = function (element) {
        $(element).addClass('rcpagejumper')
            .on('focus', function (e) {
                ...
                for (i = 1; i <= ref.env.pagecount; i++) { html += '<li>' + i + '</li>'; }
                ...
                if (!ref.pagejump) {
                    ref.pagejump = $('<div id="pagejump-selector" class="popupmenu"></div>')
                        ...
                        .on('click', 'li', function () {
                            if (!ref.busy) { $(element).val($(this).text()).change(); }
                        });
                }

                if (ref.pagejump.data('count') != i) { ref.pagejump.html(html); }
        ...
        // keyboard navigation
            .on('keydown keyup click', function (e) {
                ...
                if (e.which != 27 && e.which != 9 && e.which != 13 && !selector.is(':visible')) { return ref.show_menu('pagejump-selector', true, e); }

                if (e.type == 'keydown') {
                    // arrow-down
                    if (e.which == 40) {
                        if (list.length > p) { this.value = (p += 1); }
                    }
                    // arrow-up
                    else if (e.which == 38) {
                        if (p > 1 && list.length > p - 1) { this.value = (p -= 1); }
                    }
                    // enter
                    else if (e.which == 13) {
                        return $(this).change();
    ...
    this.mailvelope_init = function (action, keyring) {
        ...
        if (action == 'show' || action == 'preview' || action == 'print') {
            ...
            if (sign_supported) { this.env.compose_commands.push('compose-encrypted-signed'); }
            ...
            if (this.env.pgp_mime_message) {
                ...
                $.ajax({
                    ...
                    error: function (o, status, err) {
                        ...
                        if (sign_supported) { ref.enable_command('compose-encrypted-signed', !is_html); }
                    },
                    success: function (data) {
                ...
                if (sign_supported) { this.enable_command('compose-encrypted-signed', !is_html); }
            }
            ...
            this.addEventListener('actionafter', function (args) {
                if (args.ret && args.action == 'toggle-editor') {
                    ...
                    if (sign_supported) { ref.enable_command('compose-encrypted-signed', !args.props.html); }
                }
            });
    ...
    this.compose_encrypted = function (props) {
        ...
        // embed Mailvelope editor container
            ...
            if (this.spellcheck_state()) { this.editor.spellcheck_stop(); }
    ...
    this.mailvelope_display_container = function (selector, data, keyring, msgid) {
        ...
        mailvelope.createDisplayContainer(selector, data, keyring, { senderAddress: this.env.sender }).then(function (status) {
            ...
            if (ref.env.pgp_mime_part) { $('#attach' + ref.env.pgp_mime_part).remove(); }
    ...
    this.mailvelope_identity_keygen = function () {
        ...
        if (!container.length || !identity_email || !this.mailvelope_keyring.createKeyGenContainer) { return; }
    ...
    this.expunge_mailbox = function (mbox) {
        ...
        if (mbox == this.env.mailbox) {
            ...
            post_data._reload = 1;
            if (this.env.search_request) { post_data._search = this.env.search_request; }
        }
    ...
    // get message uid and folder
        var uid = this.get_single_uid(),
            ...
            submit_func = function () {
                ...
                if (typeof form.form != 'object') { return false; }

                if (!form.rc.check_compose_address_fields(post_func, form.form)) { return false; }
    ...
    this.open_compose_step = function (p) {
        ...
        if (this.env.compose_extwin && !this.env.extwin) {
            ...
            this.redirect(url);
            if (this.env.extwin) { window.resizeTo(Math.max(this.env.popup_width, $(window).width()), $(window).height() + 24); }
        }
    };
    ...
    this.init_messageform = function () {
        ...
        if (opener_rc && opener_rc.env.action == 'compose') {
            setTimeout(function () {
                if (opener.history.length > 1) { opener.history.back(); } else { opener_rc.redirect(opener_rc.get_task_url('mail')); }
            }, 100);
        ...
        if (!html_mode) {
            ...
            if (input_message.value && input_message.defaultValue !== undefined) { input_message.value = input_message.defaultValue; }
        ...
        if (this.env.save_localstorage) { this.compose_restore_dialog(0, html_mode); }

        if (input_to.val() == '') { elem = input_to; } else if (input_subject.val() == '') { elem = input_subject; } else if (input_message) { elem = input_message; }
    ...
    this.compose_restore_dialog = function (j, html_mode) {
        ...
        var show_next = function (i) {
            if (++i < index.length) { ref.compose_restore_dialog(i, html_mode); }
        };
    ...
    this.submit_messageform = function (draft, saveonly) {
        ...
        if (lang) { form.action = this.add_url(form.action, '_lang', lang); }
        if (saveonly) { form.action = this.add_url(form.action, '_saveonly', 1); }
    ...
    this.compose_recipient_select = function (list) {
        ...
        for (n = 0; n < selection.length; n++) {
            id = selection[n];
            if (this.env.contactdata[id]) { recipients++; }
        }
    ...
    // find last focused field name
        ...
        if (recipients.length && input.length) {
            ...
            if (oldval && !/[,;]\s*$/.test(oldval)) { oldval += ', '; }
    ...
    this.check_compose_input = function (cmd) {
        ...
        if (!this.check_compose_address_fields(cmd)) { return false; }
    ...
    // @todo: this should work also with many editors on page
        ...
        if (result) { mode = props.html ? 'html' : 'plain'; } else { mode = props.html ? 'plain' : 'html'; }

        // update internal format flag
        $("[name='_is_html']").val(mode == 'html' ? 1 : 0);

        if (control.is('[type=checkbox]')) { control.prop('checked', mode == 'html'); } else { control.val(mode); }
    ...
    this.set_draft_id = function (id) {
        if (id && id != this.env.draft_id) {
            ...
            if (rc && rc.env.mailbox == this.env.drafts_mailbox) { rc.command('checkmail'); }
    ...
    this.auto_save_start = function () {
        ...
        if (!this.local_save_timer && window.localStorage && this.env.save_localstorage) {
            ...
            $(window).on('unload', function () {
                ...
                if (!ref.env.server_error) { ref.remove_compose_data(ref.env.compose_id); }
            });
        }
    ...
    // check input fields
        ...
        for (i = 0; i < hash_fields.length; i++) {
            if (val = $('[name="_' + hash_fields[i] + '"]').val()) { str += val + ':'; }
        }

        str += this.editor.get_content({ refresh: false });

        for (id in this.env.attachments) { str += id; }
    ...
    // feature is disabled
        if (!this.env.save_localstorage) { return; }
        ...
        $('input, select, textarea', this.gui_objects.messageform).each(function (i, elem) {
            switch (elem.tagName.toLowerCase()) {
                case 'input':
                    ...
                    if (formdata[elem.name] != '' && elem.type != 'hidden') { empty = false; }
                    break;
                ...
                default:
                    ...
                    if (formdata[elem.name] != '') { empty = false; }
            }
        });
    ...
    this.change_identity = function (obj, show) {
        if (!obj || !obj.options) { return false; }
        ...
        if (got_sig) {
            ...
            got_sig = true;
        } else { this.enable_command('insert-sig', false); }

        // first function execution
        if (!this.env.identities_initialized) {
            this.env.identities_initialized = true;
            if (this.env.show_sig_later) { this.env.show_sig = true; }
            if (this.env.opened_extwin) { return; }
        }
        ...
        $.each(['replyto', 'bcc'], function () {
            ...
            if (old_val || new_val) { input.val(input_val).change(); }
        });

        if (this.editor) { this.editor.change_signature(id, show_sig); }

        if (show && got_sig) { this.display_message('siginserted', 'confirmation'); }
    ...
    this.add2attachment_list = function (name, att, upload_id) {
        if (upload_id) { this.triggerEvent('fileuploaded', { name: name, attachment: att, id: upload_id }); }

        if (upload_id && this.env.attachments[upload_id]) { delete this.env.attachments[upload_id]; }
        ...
        if (!att.complete && att.html.indexOf('<') < 0) { att.html = '<span class="uploading">' + att.html + '</span>'; }

        if (!att.complete && this.env.loadingicon) { att.html = '<img src="' + this.env.loadingicon + '" class="uploading" />' + att.html; }
    ...
    this.remove_attachment = function (name) {
        if (name && this.env.attachments[name]) { this.http_post('remove-attachment', { _id:this.env.compose_id, _file:name }); }

        return false;
    };

    this.cancel_attachment_upload = function (name) {
        if (!name || !this.uploads[name]) { return false; }
    ...
    this.add_contact = function (value, reload, source) {
        if (value) { this.http_post('addcontact', { _address: value, _reload: reload, _source: source }); }
    };
    ...
    // e.g. that qsearch() with no argument will execute the search.
        if (value != '' || $(this.gui_objects.qsearchbox).val() || $(this.gui_objects.search_interval).val()) {
            ...
            if (this.message_list) { this.clear_message_list(); } else if (this.contact_list) { this.list_contacts_clear(); }

            if (this.env.source) { url._source = this.env.source; }
    ...
    this.search_params = function (search, filter) {
        ...
        if (!filter && this.gui_objects.search_filter) { filter = this.gui_objects.search_filter.value; }

        if (!search && this.gui_objects.qsearchbox) { search = this.gui_objects.qsearchbox.value; }

        if (this.gui_objects.search_interval) { url._interval = $(this.gui_objects.search_interval).val(); }

        if (search) {
            url._q = search;

            if (mods && this.message_list) { mods = mods[mbox] || mods['*']; }

            if (mods) {
                for (n in mods) { mods_arr.push(n); }
    ...
    this.reset_search_filter = function () {
        ...
        if (this.gui_objects.search_filter) { $(this.gui_objects.search_filter).val('ALL').change(); }
    ...
    this.reset_qsearch = function (all) {
        ...
        if (this.gui_objects.search_interval) { $(this.gui_objects.search_interval).val(''); }

        if (this.env.qsearch) { this.abort_request(this.env.qsearch); }
    ...
    this.set_searchmods = function (mods) {
        ...
        if (!this.env.search_mods) { this.env.search_mods = {}; }

        if (mbox) { this.env.search_mods[mbox] = mods; }
    };
    ...
    this.sent_successfully = function (type, msg, folders, save_error) {
        ...
        if (this.env.extwin) {
            if (!save_error) { this.lock_form(this.gui_objects.messageform); }
            ...
            if (!save_error) { setTimeout(function () { window.close(); }, 1000); }
        } else if (!save_error) {
            // before redirect we need to wait some time for Chrome (#1486177)
            setTimeout(function () { ref.list_mailbox(); }, 500);
        }

        if (save_error) { this.env.is_sent = true; }
    };
    ...
    this.apply_image_style = function () {
        ...
        $.each({ scale: '', rotate: 'deg' }, function (i, v) {
            ...
            if (val) { style.push(i + '(' + val + v + ')'); }
        });

        if (style) { head.append($('<style id="image-style">').text('img { transform: ' + style.join(' ') + '}')); }
    };
    ...
    this.ksearch_keydown = function (e, obj, props) {
        if (this.ksearch_timer) { clearTimeout(this.ksearch_timer); }
        ...
        switch (key) {
            ...
            case 40: // arrow down
                ...
                if (!highlight) { highlight = this.ksearch_pane.__ul.firstChild; }

                if (highlight) { this.ksearch_select(dir ? highlight.previousSibling : highlight.nextSibling); }
    ...
    this.insert_recipient = function (id) {
        if (id === null || !this.env.contacts[id] || !this.ksearch_input) { return; }
    ...
    this.ksearch_get_results = function (props) {
        if (this.ksearch_pane && this.ksearch_pane.is(':visible')) { this.ksearch_pane.hide(); }
        ...
        if (q == this.ksearch_value) { return; }
        ...
        if (old_value && old_value.length && q.startsWith(old_value) && (!data || data.num <= 0) && this.env.contacts && !this.env.contacts.length) { return; }
    ...
    // trigger multi-thread http response callback
        ...
        if (!this.ksearch_value) { return; }

        // ignore this outdated search response
        if (this.ksearch_input && search != this.ksearch_value) { return; }
        ...
        if (len) { this.env.contacts = this.env.contacts.concat(results); }

        if (this.ksearch_data.id == reqid) { this.ksearch_data.num--; }
    };
    ...
    this.ksearch_input_replace = function (from, to, input, trigger) {
        if (!this.ksearch_input && !input) { return; }

        if (!input) { input = this.ksearch_input; }
    ...
    this.ksearch_click = function (node) {
        if (this.ksearch_input) { this.ksearch_input.focus(); }
    ...
    this.ksearch_hide = function () {
        ...
        if (this.ksearch_pane) { this.ksearch_pane.hide(); }
    ...
    this.ksearch_destroy = function () {
        if (this.ksearch_data) { this.multi_thread_request_abort(this.ksearch_data.id); }

        if (this.ksearch_info) { this.hide_message(this.ksearch_info); }

        if (this.ksearch_msg) { this.hide_message(this.ksearch_msg); }
    ...
    this.contactlist_select = function (list) {
        ...
        if (this.env.contentframe && !list.multi_selecting && (id = list.get_single_selection())) { this.preview_timer = setTimeout(function () { ref.load_contact(id, 'show'); }, this.preview_delay_click); } else if (this.env.contentframe) { this.show_contentframe(false); }
        ...
        if (selected) {
            ...
            $.each(list.get_selection(), function (i, v) {
                ...
                if (contact._type != 'group') { list.draggable = true; }
            });

            this.env.selection_sources = $.unique(this.env.selection_sources);

            if (source && source.groups) { $.each(this.env.contactgroups, function () { if (this.source === ref.env.source) { groupcount++; } }); }
    ...
    this.list_contacts = function (src, group, page, search) {
        ...
        if (!src) { src = this.env.source; }

        if (refresh) { group = this.env.group; }

        if (src != this.env.source) {
            page = this.env.current_page = 1;
            this.reset_qsearch();
        } else if (!refresh && group != this.env.group) { page = this.env.current_page = 1; }

        if (this.env.search_id) { folder = 'S' + this.env.search_id; } else if (!this.env.search_request) { folder = group ? 'G' + src + group : src; }
        ...
        if (!this.env.search_id) { this.select_folder(folder, '', true); }
        ...
        if (group) { url._gid = group; }
        if (page) { url._page = page; }
        if (src) { url._source = src; }
    ...
    // clear message list first
        ...
        if (this.env.task != 'mail') { this.update_state({ _source: src, _page: page && page > 1 ? page : null, _gid: group }); }
    };
    ...
    this.load_contact = function (cid, action, framed) {
        ...
        if (win = this.get_frame_window(this.env.contentframe)) {
            ...
            if (!cid) { this.contact_list.clear_selection(); }
        ...
        if (action && (cid || action == 'add') && !this.drag_active) {
            ...
            if (cid) { url._cid = this.preview_id = cid; }
    ...
    this.contacts_drag_menu = function (e, to) {
        ...
        if (!this.env.address_sources[dest] || this.env.address_sources[dest].readonly) { return true; }

        // search result may contain contacts from many sources, but if there is only one...
        if (source == '' && this.env.selection_sources.length == 1) { source = this.env.selection_sources[0]; }
    ...
    this.copy_contacts = function (to, event, cid) {
        ...
        if (!cid || !this.env.address_sources[dest] || this.env.address_sources[dest].readonly) { return; }
    ...
    this.move_contacts = function (to, event, cid) {
        ...
        if (!this.env.address_sources[dest] || this.env.address_sources[dest].readonly) { return; }

        if (!cid) { cid = this.contact_list.get_selection(); }
        ...
        // target is an addressbook
        else if (to.id != source) { this._with_selected_contacts('move', { _to: to.id, _cid: cid }); }
    };
    ...
    this._with_selected_contacts = function (action, post_data) {
        ...
        if (!selection.length && !this.env.cid) { return; }
        ...
        if (this.env.cid) { a_cids.push(this.env.cid); } else {
            for (n = 0; n < selection.length; n++) {
                id = selection[n];
                a_cids.push(id);
            ...
            if (!display_next) { this.contact_list.clear_selection(); }
        }
        ...
        if (this.env.group) { post_data._gid = this.env.group; }
    ...
    this.update_contact_row = function (cid, cols_arr, newcid, source, data) {
        ...
        if (!list.rows[cid]) {
            cid = cid + '-' + source;
            if (newcid) { newcid = newcid + '-' + source; }
        }
    ...
    this.add_contact_row = function (cid, cols, classes, data) {
        if (!this.gui_objects.contactslist) { return false; }
        ...
        if (list.in_selection(cid)) { row.className += ' selected'; }
    ...
    this.init_contact_form = function () {
        ...
        if (this.env.coltypes) {
            ...
            for (col in this.env.coltypes) { this.init_edit_field(col, null); }
        }
    ...
    this.remove_group_item = function (prop) {
        ...
        if (prop.source == this.env.source && prop.id == this.env.group) { this.list_contacts(prop.source, 0); }
    };
    ...
    this.update_contact_group = function (prop) {
        ...
        // update displayed group name
            ...
            if (prop.source == this.env.source && prop.id == this.env.group) { this.set_group_prop(prop); }
        }
    ...
    this.init_edit_field = function (col, elem) {
        ...
        if (!elem) { elem = $('.ff_' + col); }

        if (label && !$('label[for="ff_' + col + '"]').length) { elem.placeholder(label); }
    };
    ...
    // just make pre-defined input field visible
        ...
        if (elem.length) {
            ...
            if (!appendcontainer.length) {
                ...
                if (lastgroup.length) { appendcontainer.insertAfter(lastgroup); } else { sect.prepend(appendcontainer); }
            }
            ...
            if (appendcontainer.get(0).nodeName == 'FIELDSET') {
                ...
                if (colprop.subtypes_select) {
                    ...
                    if (!compact) { label = $('<div>').addClass('contactfieldlabel label').append(label); } else { label.addClass('input-group-prepend'); }
                } else {
                    label = $('<label>').addClass('contactfieldlabel label input-group-text')
                        .attr('for', input_id).text(colprop.label);

                    if (compact) { label = $('<span class="input-group-prepend">').append(label); }
                }
                ...
                if (colprop.type == 'text' || colprop.type == 'date') {
                    ...
                    if (colprop.type == 'date' && $.datepicker) { input.addClass('datepicker').datepicker(); }
                    ...
                    if (compact) { content = $('<div class="content input-group-text">'); }
                    ...
                    for (i = 0; i < cols.length; i++) {
                        ...
                        if (!compact) { content.append(suffices[i] || ' '); }
                    ...
                    if (colprop.options) { $.each(colprop.options, function (i, val) { options[options.length] = new Option(val, i); }); }
                }
                ...
                if (input) {
                    ...
                    if (!compact) {
                        if (colprop.type != 'composite') { cell.append(input); }
                    ...
                    if (input.is('div')) { input.find('input').first().focus(); } else { input.first().focus(); }

                    // disable option if limit reached
                    if (!colprop.count) { colprop.count = 0; }
                    if (++colprop.count == colprop.limit && colprop.limit) { $(menu).children('option[value="' + col + '"]').prop('disabled', true); }
    ...
    this.delete_edit_field = function (elem) {
        ...
        if (--colprop.count <= 0 && colprop.visible) { input_group.find('input').val('').blur(); } else {
            input_group.remove();
            // hide entire fieldset if no more rows
            if (!fieldset.children('div.row').length) { fieldset.hide(); }
        }

        // enable option in add-field selector or insert it if necessary
        if (addmenu.length) {
            var option = addmenu.children('option[value="' + col + '"]');
            if (option.length) { option.prop('disabled', false); } else { option = $('<option>').attr('value', col).html(colprop.label).appendTo(addmenu); }
            addmenu.show();
        }
    };
    ...
    this.set_photo_actions = function (id) {
        ...
        for (n = 0; buttons && n < buttons.length; n++) { $('a#' + buttons[n].id).html(this.get_label(id == '-del-' ? 'addphoto' : 'replacephoto')); }
    ...
    this.listsearch = function (id) {
        ...
        } else { this.select_folder('S' + id, '', true); }
    ...
    this.load_response = function (id, action) {
        ...
        if (win = this.get_frame_window(this.env.contentframe)) {
            if (id || action == 'add-response') {
                if (!id) { this.responses_list.clear_selection(); }
    ...
    this.load_identity = function (id, action) {
        ...
        if (win = this.get_frame_window(this.env.contentframe)) {
            if (id || action == 'add-identity') {
                if (!id) { this.identity_list.clear_selection(); }
    ...
    this.delete_identity = function (id) {
        if (!id && this.identity_list) { id = this.identity_list.get_single_selection(); }
        ...
        this.subscription_list
            ...
            .droppable({
                ...
                accept: function (node) {
                    if (!node.is('.mailbox')) { return false; }
    ...
    this.add_folder_row = function (id, name, display_name, is_protected, subscribed, class_name, refrow, subfolders) {
        ...
        if (this.subscription_list.is_draggable()) { this.subscription_list.draggable('destroy').droppable('destroy'); }
        ...
        folders.sort(function (a, b) {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (f1 !== f2) {
                    if (f2 === undefined) { return 1; }
                    if (collator) { return collator.compare(f1, f2); } else { return f1 < f2 ? -1 : 1; }
                } else if (i == len - 1) {
                    return -1;
                }
            }
        ...
        for (n in folders) {
            ...
            if (folders[n][2]) {
                ...
                if (tmp_name == this.env.prefix_ns) { continue; }
                slist.push(p);
                tmp = tmp_name;
            }
            // protected folder's child
            else if (tmp && p.startsWith(tmp)) { slist.push(p); }
            // other
            else {
                list.push(p);
                tmp = null;
        ...
        for (n = 0; n < slist.length; n++) {
            if (id.startsWith(slist[n] + this.env.delimiter)) { rowid = slist[n]; }
        }

        // find folder position after sorting
        for (n = 0; !rowid && n < list.length; n++) {
            if (n && list[n] == id) { rowid = list[n - 1]; }
        }
        ...
        if (!refrow) { this.triggerEvent('clonerow', { row: row, id: id }); }
    ...
    this.show_folder = function (folder, path, force) {
        ...
        if (path) { url += '&_path=' + urlencode(path); }
        ...
        if (String(target.location.href).indexOf(url) >= 0 && !force) { this.show_contentframe(true); } else { this.location_href(this.env.comm_path + url, target, true); }
    };
    ...
    this.disable_subscription = function (folder) {
        ...
        if (row) { $('input[name="_subscribed[]"]', row).first().prop('disabled', true); }
    };

    // resets state of subscription checkbox (e.g. on error)
    this.reset_subscription = function (folder, state) {
        var row = this.subscription_list.get_item(folder, true);
        if (row) { $('input[name="_subscribed[]"]', row).first().prop('checked', state); }
    };
    ...
    this.init_button = function (cmd, prop) {
        ...
        if (prop.sel) {
            ...
            if (preload) { new Image().src = prop.sel; }
        }
        if (prop.over) {
            ...
            if (preload) { new Image().src = prop.over; }
        }
    };
    ...
    this.set_button = function (command, state) {
        ...
        for (n = 0; n < len; n++) {
            ...
            if (!obj || button.status === state) { continue; }

            // get default/passive setting of the button
            if (button.type == 'image' && !button.status) {
                button.pas = obj._original_src ? obj._original_src : obj.src;
                // respect PNG fix on IE browsers
                if (obj.runtimeStyle && obj.runtimeStyle.filter && obj.runtimeStyle.filter.match(/src=['"]([^'"]+)['"]/)) { button.pas = RegExp.$1; }
            } else if (!button.status) { button.pas = String(obj.className); }
    ...
    this.set_alttext = function (command, label) {
        ...
        for (n = 0; n < len; n++) {
            ...
            if (obj && button.type == 'image') {
                ...
                if ((link = obj.parentNode) && link.tagName.toLowerCase() == 'a') { link.setAttribute('title', label); }
            } else if (obj) { obj.setAttribute('title', label); }
        }
    };
    ...
    this.display_message = function (msg, type, timeout, key) {
        if (msg && msg.length && /^[a-z._]+$/.test(msg)) { msg = this.get_label(msg); }

        // pass command to parent window
        if (this.is_framed()) { return parent.rcmail.display_message(msg, type, timeout); }

        if (!this.gui_objects.message) {
            // save message in order to display after page loaded
            if (type != 'loading') { this.pending_message = [msg, type, timeout, key]; }
            return 1;
        }

        if (!type) { type = 'notice'; } else if (type == 'loading') {
            if (!key) { key = 'loading'; }
            if (!timeout) { timeout = this.env.request_timeout * 1000; }
            if (!msg) { msg = this.get_label('loading'); }
        }

        if (!key) { key = this.html_identifier(msg); }
        ...
        if (this.messages[key]) {
            // replace label
            if (this.messages[key].obj) { $('div.content', this.messages[key].obj).html(msg); }
        ...
        if (timeout > 0) { setTimeout(function () { ref.hide_message(id, type != 'loading'); }, timeout); }
    ...
    // pass command to parent window
        if (this.is_framed()) { return parent.rcmail.hide_message(obj, fade); }

        if (!this.gui_objects.message) { return; }
    ...
    this.hide_message_object = function (o, fade) {
        if (fade) { o.fadeOut(600, function () { $(this).remove(); }); } else { o.hide().remove(); }
    };
    ...
    // pass command to parent window
        ...
        for (k in m) {
            for (n in m[k].elements) {
                if (m[k].obj) { this.hide_message_object(m[k].obj); }
            }
        }
    ...
    this.display_progress = function (data) {
        ...
        if (!data.label) { data.label = this.get_label('uploadingmany'); }

        if (!msg) {
            if (!data.percent || data.percent < 100) { this.display_message(data.label, 'uploading', 0, 'progress' + data.name); }
            return;
        }
        ...
        if (data.text) { data.label += ' ' + data.text; }
    ...
    // forward call to parent window
        ...
        if (typeof content == 'object') {
            ...
            if ($(content).is('iframe')) { popup.addClass('iframe'); }
        } else { popup.html(content); }
        ...
        if (options && options.button_classes) {
            $.each(buttons, function (idx, button) {
                ...
                if (cl) { buttons[idx] = fn(button, cl, idx); }
                i++;
            });
        }
        ...
        if (options.width) { popup.width(options.width); }
        if (options.height) { popup.height(options.height); }
    ...
    this.simple_dialog = function (content, title, action_func, options) {
        ...
        if (!action_func) { buttons[0]['class'] += ' mainaction'; } else {
            buttons.unshift({
    ...
    // update list mode columns list
        ...
        if (thead) {
            if (repl) {
                ...
                if (list.checkbox_selection) { list.insert_checkbox(tr, 'thead'); }
        ...
        if (this.env.coltypes.folder) { this.env.coltypes.folder.hidden = !(this.env.search_request || this.env.search_id) || this.env.search_scope == 'base'; }

        if ((n = $.inArray('subject', listcols)) >= 0) {
            this.env.subject_col = n;
            if (list) { list.subject_col = n; }
        }
        if ((n = $.inArray('flag', listcols)) >= 0) { this.env.flagged_col = n; }
        if ((n = $.inArray('status', listcols)) >= 0) { this.env.status_col = n; }
    ...
    this.set_mailboxname = function (content) {
        if (this.gui_objects.mailboxname && content) { this.gui_objects.mailboxname.innerHTML = content; }
    };

    // replace content of quota display
    this.set_quota = function (content) {
        if (this.gui_objects.quotadisplay && content && content.type == 'text') { $(this.gui_objects.quotadisplay).text((content.percent || 0) + '%').attr('title', content.title || ''); }
    ...
    this.set_unread_count = function (mbox, count, set_title, mark) {
        ...
        if (mark) { this.mark_folder(mbox, mark, '', true); } else if (!count) { this.unmark_folder(mbox, 'recent', '', true); }
    ...
    this.set_unread_count_display = function (mbox, set_title) {
        ...
        if (item = this.get_folder_li(mbox, '', true)) {
            ...
            if (!text_obj.length && mycount) { text_obj = $('<span>').addClass('unreadcount skip-content').appendTo(link); }
            reg = /\s+\([0-9]+\)$/i;
            ...
            if ((div = item.getElementsByTagName('div')[0])
                ...
                for (var k in this.env.unread_counts) {
                    if (k.startsWith(mbox + this.env.delimiter)) { childcount += this.env.unread_counts[k]; }
                }
            }

            if (mycount && text_obj.length) { text_obj.html(this.env.unreadwrap.replace(/%[sd]/, mycount)); } else if (text_obj.length) { text_obj.remove(); }

            // set parent's display
            reg = new RegExp(RegExp.escape(this.env.delimiter) + '[^' + RegExp.escape(this.env.delimiter) + ']+$');
            if (mbox.match(reg)) { this.set_unread_count_display(mbox.replace(reg, ''), false); }

            // set the right classes
            if ((mycount + childcount) > 0) { $(item).addClass('unread'); } else { $(item).removeClass('unread'); }
        }
        ...
        if (set_title && document.title) {
            ...
            if (mycount && doc_title.match(reg)) { new_title = doc_title.replace(reg, '(' + mycount + ') '); } else if (mycount) { new_title = '(' + mycount + ') ' + doc_title; } else { new_title = doc_title.replace(reg, ''); }
    ...
    this.set_headers = function (content) {
        if (this.gui_objects.all_headers_box && content) { $(this.gui_objects.all_headers_box).html(content).show(); }
    };

    // display all-headers row and fetch raw message headers
    this.show_headers = function (props, elem) {
        if (!this.gui_objects.all_headers_row || !this.gui_objects.all_headers_box || !this.env.uid) { return; }
    ...
    this.hide_headers = function (props, elem) {
        if (!this.gui_objects.all_headers_row || !this.gui_objects.all_headers_box) { return; }
    ...
    this.folder_selector = function (event, callback) {
        this.entity_selector('folder-selector', callback, this.env.mailboxes_list, function (obj, a) {
            ...
            if (folder.virtual) { a.addClass('virtual').attr({ 'aria-disabled': 'true', tabindex: '-1' }); } else { a.addClass('active').data('id', folder.id); }

            if (folder['class']) { row.addClass(folder['class']); }
    ...
    this.entity_selector = function (name, click_callback, entity_list, list_callback, event) {
        ...
        if (!container) {
            ...
            if (rows.length > 10) { container.css('max-height', $('li', container)[0].offsetHeight * 10 + 9); }
    ...
    this.show_menu = function (prop, show, event) {
        ...
        if (ref.get(0).tagName != 'A' && ref.closest('a').length) { ref = ref.closest('a'); }

        if (typeof prop == 'string') { prop = { menu:name }; }
        ...
        if (typeof show == 'undefined') { show = obj.is(':visible') ? false : true; }
        ...
        if (show) {
            ...
            for (var i = this.menu_stack.length - 1; stack && i >= 0; i--) {
                if (!$(ref).parents('#' + this.menu_stack[i]).length && $(event.target).parent().attr('role') != 'menuitem') { this.hide_menu(this.menu_stack[i], event); }
            }
    ...
    this.hide_menu = function (name, event) {
        ...
        if (this.menu_stack.length && keyboard) {
            ...
            if (!obj || !obj.data('opener')) { $('#' + this.focused_menu).find('a,input:not(:disabled)').not('[aria-disabled=true]').first().focus(); }
        } else {
    ...
    this.element_position = function (element, obj) {
        ...
        if (left + elem_width > win.width()) { left -= elem_width + width; }
    ...
    // warn the user (if converted content is not empty)
        if (!text
            ...
            if (func) { setTimeout(function () { func(''); }, 50); }
            return true;
        }
    ...
    this.url = function (action, query) {
        ...
        if (typeof action !== 'string') { query = action; } else if (!query || typeof query !== 'object') { query = {}; }

        if (action) { query._action = action; } else if (this.env.action) { query._action = this.env.action; }
        ...
        for (k in query) {
            if (query[k] !== undefined && query[k] !== null) { param[k] = query[k]; }
        }

        if (param = $.param(param)) { url += (url.indexOf('?') > -1 ? '&' : '?') + param; }
    ...
    this.redirect = function (url, lock) {
        if (lock !== false) { this.set_busy(true, 'loading'); }
        ...
        if (this.is_framed()) {
            ...
            if (this.env.extwin) {
                if (typeof url == 'string') { url += (url.indexOf('?') < 0 ? '?' : '&') + '_extwin=1'; } else { url._extwin = 1; }
            }
    ...
    this.location_href = function (url, target, frame) {
        if (frame) { this.lock_frame(target); }

        if (typeof url == 'object') { url = this.env.comm_path + '&' + $.param(url); }

        // simulate real link click to force IE to send referer header
        if (bw.ie && target == window) { $('<a>').attr('href', url).appendTo(document.body).get(0).click(); } else { target.location.href = url; }
    ...
    this.http_request = function (action, data, lock, type) {
        ...
        // abort if one of the handlers returned false
        if (result === false) {
            if (data._unlock) { this.set_busy(false, null, data._unlock); }
            return false;
    ...
    this.abort_request = function (r) {
        if (r.request) { r.request.abort(); }
        if (r.lock) { this.set_busy(false, null, r.lock); }
    };
    ...
    this.http_response = function (response) {
        ...
        if (response.unlock) { this.set_busy(false, null, response.unlock); }

        this.triggerEvent('responsebefore', { response: response });
        this.triggerEvent('responsebefore' + response.action, { response: response });

        // set env vars
        if (response.env) { this.set_env(response.env); }
        ...
        if (typeof response.texts === 'object') {
            for (i in response.texts) {
                if (typeof response.texts[i] === 'string') { this.add_label(i, response.texts[i]); }
            }
        }
        ...
        if (response.callbacks && response.callbacks.length) {
            for (i = 0; i < response.callbacks.length; i++) { this.triggerEvent(response.callbacks[i][0], response.callbacks[i][1]); }
        }
        ...
        switch (response.action) {
            case 'mark':
                ...
                if ((this.env.action == 'show' || this.env.action == 'preview') && this.env.last_flag == 'SEEN') { this.set_unread_message(this.env.uid, this.env.mailbox); }
                break;
            ...
            case 'move':
                if (this.env.action == 'show') {
                    ...
                    if (!this.env.list_post) { this.enable_command('reply-list', false); }
                } else if (this.task == 'addressbook') {
            ...
            case 'expunge':
                if (this.task == 'mail') {
                    ...
                    if (this.message_list) { this.triggerEvent('listupdate', { list:this.message_list, folder:this.env.mailbox, rowcount:this.message_list.rowcount }); }
                }
                break;
                ...
                if (this.task == 'mail') {
                    ...
                    if (list) {
                        if (response.action == 'list' || response.action == 'search') {
                            ...
                            if (list.rowcount > 0 && !$(document.activeElement).is('input,textarea')) { list.focus(); }
                        ...
                        if (response.action != 'getunread') { this.triggerEvent('listupdate', { list:list, folder:this.env.mailbox, rowcount:list.rowcount }); }
                    }
                } else if (this.task == 'addressbook') {
                    var list = this.contact_list,
                        uid = this.env.list_uid;
            ...
            case 'search-contacts':
                if (this.contact_list) {
                    if (this.contact_list.rowcount > 0) { this.contact_list.focus(); }
                    this.triggerEvent('listupdate', { list:this.contact_list, rowcount:this.contact_list.rowcount });
                }
                break;
        }

        if (response.unlock) { this.hide_message(response.unlock); }
    ...
    this.http_error = function (request, status, err, lock, action) {
        ...
        if (request.status && errmsg) { this.display_message(this.get_label('servererror') + ' (' + errmsg + ')', 'error'); } else if (status == 'timeout') { this.display_message('requesttimedout', 'error'); } else if (request.status == 0 && status != 'abort') { this.display_message('connerror', 'error'); }
        ...
        if (action == 'keep-alive') { setTimeout(function () { ref.keep_alive(); ref.start_keepalive(); }, 30000); }
    };
    ...
    this.iframe_loaded = function (unlock) {
        if (!unlock) { unlock = this.env.frame_lock; }

        this.set_busy(false, null, unlock);

        if (this.submit_timer) { clearTimeout(this.submit_timer); }
    };
  ...
  */
    ...
    this.multi_thread_http_response = function (data, reqid) {
        ...
        if (!prop || prop.running <= 0 || prop.cancelled) { return; }
    ...
    this.multi_thread_request_abort = function (reqid) {
        ...
        if (prop) {
            for (var i = 0; prop.running > 0 && i < prop.requests.length; i++) {
                if (prop.requests[i].abort) { prop.requests[i].abort(); }
            }
    ...
    // abort event and reset UI
        ...
        if (!files || !files.length) {
            ...
            if (uri = e.dataTransfer.getData('roundcube-uri')) {
                ...
                if (!this.add2attachment_list(ts, { name: '', html: content, classname: 'uploading', complete: false })) { this.file_upload_id = this.set_busy(true, 'attaching'); }
    ...
    this.file_upload = function (files, post_args, props) {
        if (!window.FormData || !files || !files.length) { return false; }
        ...
        if (numfiles) {
            ...
            // add to attachments list
            if (!this.add2attachment_list(ts, { name: '', html: content, classname: 'uploading', complete: false }) && !props.lock) { props.lock = this.file_upload_id = this.set_busy(true, 'uploading'); }
    ...
    this.start_keepalive = function () {
        if (!this.env.session_lifetime || this.env.framed || this.env.extwin || this.task == 'login' || this.env.action == 'print') { return; }
    ...
    this.start_refresh = function () {
        if (!this.env.refresh_interval || this.env.framed || this.env.extwin || this.task == 'login' || this.env.action == 'print') { return; }
    ...
    this.keep_alive = function () {
        if (!this.busy) { this.http_request('keep-alive'); }
    };
    ...
    this.refresh = function () {
        ...
        if (this.task == 'mail' && this.gui_objects.mailboxlist) { params = this.check_recent_params(); }
    ...
    this.check_recent_params = function () {
        ...
        if (this.gui_objects.mailboxlist) { params._folderlist = 1; }
        if (this.gui_objects.quotadisplay) { params._quota = 1; }
        if (this.env.search_request) { params._search = this.env.search_request; }
   ...
   */
    ...
    this.opener = function (deep, filter) {
        ...
        try {
            if (win && !win.closed && win !== window) {
                ...
                if (deep && (!win.rcmail || win.rcmail.env.framed) && win.parent && win.parent.rcmail) { win = win.parent; }

                if (win.rcmail && filter) {
                    for (i in filter) {
                        if (win.rcmail.env[i] != filter[i]) { return; }
                    }
                }
    ...
    this.get_message_mailbox = function (uid) {
        ...
        if (this.env.messages && uid && (msg = this.env.messages[uid]) && msg.mbox) { return msg.mbox; }

        if (/^[0-9]+-(.*)$/.test(uid)) { return RegExp.$1; }
    ...
    this.get_caret_pos = function (obj) {
        if (obj.selectionEnd !== undefined) { return obj.selectionEnd; }
    ...
    this.set_caret_pos = function (obj, pos) {
        try {
            if (obj.setSelectionRange) { obj.setSelectionRange(pos, pos); }
    ...
    this.lock_form = function (form, lock) {
        ...
        $.each(form.elements, function () {
            ...
            if (lock && this.disabled) { ref.disabled_form_elements.push(this); } else if (lock || $.inArray(this, ref.disabled_form_elements) < 0) { this.disabled = lock; }
        });
    };
    ...
    this.check_protocol_handler = function (name, elem) {
        ...
        if (!nav || (typeof nav.registerProtocolHandler != 'function')) {
            ...
            if (status) { $(elem).parent().find('.mailtoprotohandler-status').html(status); }
        } else {
    ...
    this.browser_capabilities_check = function () {
        ...
        $.each(['pdf', 'flash', 'tiff', 'webp', 'pgpmime'], function () {
            if (ref.env.browser_capabilities[this] === undefined) { ref.env.browser_capabilities[this] = ref[this + '_support_check'](); }
        });
    };
    ...
    this.browser_capabilities = function () {
        ...
        for (n in this.env.browser_capabilities) { ret.push(n + '=' + this.env.browser_capabilities[n]); }
    ...
    this.pdf_support_check = function () {
        if ('pdfViewerEnabled' in navigator) { return navigator.pdfViewerEnabled ? 1 : 0; }
    ...
    this.flash_support_check = function () {
        ...
        if ('ActiveXObject' in window) {
            try {
                if (plugin = new ActiveXObject('ShockwaveFlash.ShockwaveFlash')) { return 1; }
            } catch (e) {}
        }
    ...
    this.get_local_storage_prefix = function () {
        if (!this.local_storage_prefix) { this.local_storage_prefix = 'roundcube.' + (this.env.user_id || 'anonymous') + '.'; }
...
rcube_webmail.long_subject_title = function (elem, indent, text_elem) {
    if (!elem.title) {
        ...
        if ($elem.width() + (indent || 0) * 15 > $elem.parent().width()) { elem.title = rcube_webmail.subject_text($elem[0]); }
    }
};
...
rcube_webmail.long_subject_title_ex = function (elem) {
    if (!elem.title) {
        ...
        tmp.remove();
        if (w + indent * 15 > $elem.width()) { elem.title = rcube_webmail.subject_text(elem); }
    }
};
    ...
    $('iframe').each(function () {
        ...
        $.each(events, function (event_name, event_handler) {
            frame.on('load', function (e) {
                try { $(this).contents().on(event_name, event_handler); } catch (e) { /* catch possible permission error in IE */ }
            });

            try { frame.contents().on(event_name, event_handler); } catch (e) { /* catch possible permission error in IE */ }
        });
    });
};
