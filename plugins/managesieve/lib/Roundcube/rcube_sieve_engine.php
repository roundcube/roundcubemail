...
class rcube_sieve_engine
{
    ...
    function actions()
    {
        ...
        // If we don't have an action, initialize the form
        if (empty($action)) {
        ...
        // Some actions do not permit an error to have happened
        ...
        // Some actions require a script rule to be set
        $scriptcheck = array('delete', 'move', 'act');

        $scriptset = isset($this->script[$fid]);

        if (in_array($action, $scriptcheck) && !$scriptset) {
            $this->rc->output->send();
        ...
        switch ($action) {
            ...
            case 'act':
                ...
                if ($result !== false) $result = $this->save_script();
    ...
    function save()
    {
        ...
        if ($max_post || $max_depth) {
            if (count($_POST, COUNT_RECURSIVE) >= $max_post) {
                rcube::raise_error(
                    array(
                        'code'    => 500, 'type' => 'php',
                        'file'    => __FILE__, 'line' => __LINE__,
            ...
            if (count($_POST['_header']) > $max_depth) {
    ...
    {
        if (!$attrib['id']) $attrib['id'] = 'rcmfilterframe';

        $attrib['name'] = $attrib['id'];

        $this->rc->output->set_env('contentframe', $attrib['name']);

        $this->rc->output->set_env(
            'blankpage',
            $attrib['src'] ? $this->rc->output->abs_url($attrib['src']) : 'program/resources/blank.gif'
        );
    ...
    {
        if (!$attrib['id']) $attrib['id'] = 'rcmfiltersetform';
        ...
        $out .= sprintf(
            ...
            '_name',
            rcube::Q($this->plugin->gettext('filtersetname')),
            $input_name->show($name)
        );
        ...
        if (is_array($list)) {
            ...
            if (!$copy) $copy = $_SESSION['managesieve_current'];
    ...
    {
        if (!$attrib['id']) $attrib['id'] = 'rcmfilterform';
        ...
        $input_name = new html_inputfield(array(
            'name'  => '_name', 'id' => $field_id, 'size' => 30,
            'class' => ($this->errors['name'] ? 'error' : '')
        ));
        ...
        $input_join = new html_radiobutton(array(
            'name'    => '_join', 'id' => $field_id, 'value' => 'allof',
        ...
        $out .= sprintf(
            ...
            $input_join,
            $field_id,
            rcube::Q($this->plugin->gettext('filterallof'))
        );

        $field_id = '_anyof';

        $input_join = new html_radiobutton(
            array(
                'name'    => '_join',
                'id'      => $field_id,
                'value'   => 'anyof',
                'onclick' => 'rule_join_radio(\'anyof\')',
                'class'   => 'radio'
            )
        );
        ...
        $out .= sprintf(
            ...
            $input_join,
            $field_id,
            rcube::Q($this->plugin->gettext('filteranyof'))
        );
        ...
        $out .= sprintf(
            ...
            $input_join,
            $field_id,
            rcube::Q($this->plugin->gettext('filterany'))
        );
    ...
    function rule_div($fid, $id, $div = true)
    {
        if (isset($this->form)) {
            ...
            $rows_num = sizeof($this->form['tests']);
        } else {
            $rule     = $this->script[$fid]['tests'][$id];
            $rows_num = sizeof($this->script[$fid]['tests']);
        }

        // headers select
        $select_header = new html_select(
            array(
                'name'     => "_header[]",
                'id'       => 'header' . $id,
        ...
        if (isset($rule['test'])) {
            if (
                in_array($rule['test'], array('header', 'address', 'envelope'))
                && !is_array($rule['arg1'])
                && in_array($rule['arg1'], $this->headers)
            ) {
                $test = $rule['arg1'];
            } else if (
                $rule['test'] == 'exists'
                && !is_array($rule['arg'])
                && in_array($rule['arg'], $this->headers)
            ) {
                $test = $rule['arg'];
        ...
        $tout = $this->list_input(
                $id,
                'custom_header',
                $custom,
                isset($custom),
                $this->error_class($id, 'test', 'header', 'custom_header'),
                15
            ) . "\n";
    ...
    function action_div($fid, $id, $div = true)
    {
        if (isset($this->form)) {
            ...
            $rows_num = sizeof($this->form['actions']);
        } else {
            $action   = $this->script[$fid]['actions'][$id];
            $rows_num = sizeof($this->script[$fid]['actions']);
        }
    ...
    private function error_class($id, $type, $target, $elem_prefix = '')
    {
        // TODO: tooltips
        if (
            ($type == 'test' && ($str = $this->errors['tests'][$id][$target]))
    ...
    {
        if ($error) $str = html::span('sieve error', $str);
        ...
        if ($this->rc->config->get('managesieve_kolab_master')) {
            ...
            if ($user_script && array_search($name, $this->active) === false) {
                ...
                if ($this->sieve->load($user_script)) {
                    ...
                    if ($result) $this->active[] = $name;
                }
            }
        } else {
            $result = $this->sieve->activate($name);

            if ($result) $this->active = array($name);
        }
