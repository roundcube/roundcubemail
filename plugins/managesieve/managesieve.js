        ...
        if (rcmail.env.action == 'plugin.managesieve' || rcmail.env.action == 'plugin.managesieve-save') {
            ...
            if (rcmail.gui_objects.filterslist) {
                rcmail.filters_list = new rcube_list_widget(
                    rcmail.gui_objects.filterslist,
...
rcube_webmail.prototype.managesieve_del = function ()
{
    ...
    if (confirm(this.get_label('managesieve.filterdeleteconfirm'))) {
        ...
        this.http_post(
            'plugin.managesieve',
            '_act=delete&_fid=' + this.filters_list.rows[id].uid,
            lock
        );
    }
};
...
rcube_webmail.prototype.managesieve_act = function ()
{
    ...
    this.http_post(
        'plugin.managesieve',
        '_act=act&_fid=' + this.filters_list.rows[id].uid,
        lock
    );
};
...
rcube_webmail.prototype.managesieve_select = function (list)
{
    ...
    if (id != null) this.load_managesieveframe(list.rows[id].uid);
};
...
rcube_webmail.prototype.managesieve_setselect = function (list)
{
    ...
    if (id != null) this.managesieve_list(this.env.filtersets[id]);
};
...
rcube_webmail.prototype.load_managesieveframe = function (id)
{
    ...
    if (this.env.contentframe && window.frames && window.frames[this.env.contentframe]) {
        ...
        target.location.href = this.env.comm_path
            + '&_action=plugin.managesieve&_framed=1'
            + (has_id ? '&_fid=' + id : '')
            + '&_unlock=' + msgid;
    }
};
...
rcube_webmail.prototype.managesieve_dragend = function (e)
{
    if (this.drag_active) {
        if (this.drag_filter_target) {
            ...
            this.http_post(
                'plugin.managesieve',
                '_act=move&_fid=' + this.drag_filter
                    + '&_to=' + this.drag_filter_target,
                lock
            );
        }
...
function rule_header_select(id)
{
    ...
    if (h == 'size') {
        ...
        $.each([op, header, mod, trans, comp], function () {
...
function smart_field_init(field)
{
    ...
    if (field.hasClass('error')) {
        ...
        rcmail.managesieve_tip_register([
            [id, field.data('tip')]
        ]);
    }
}
...
rcube_webmail.prototype.managesieve_create = function ()
{
    ...
    for (i in rcmail.env.sieve_headers) {
        html += '<li><input type="checkbox" name="headers[]" id="sievehdr' + i
            + '" value="' + i + '" checked="checked" />'
            + '<label for="sievehdr' + i + '">'
            + rcmail.env.sieve_headers[i][0] + ':</label> '
            + rcmail.env.sieve_headers[i][1] + '</li>';
    }
